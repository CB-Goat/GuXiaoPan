...
interface StockInfoDao {
    ...
    @Query("SELECT * FROM Stock_Info WHERE stockCode = :code")
    ...
    @Query("UPDATE Stock_Info SET flag = :flag WHERE stockCode IN (:codes)")
    suspend fun updateFlag(codes: List<String>, flag: Int)

    @Query("UPDATE Stock_Info SET concept = :concept WHERE stockCode = :code")
    suspend fun updateConcept(code: String, concept: String)

    @Query("UPDATE Stock_Info SET analystRating = :rating WHERE stockCode = :code")
