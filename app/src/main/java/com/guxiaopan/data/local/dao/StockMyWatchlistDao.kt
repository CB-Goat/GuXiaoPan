...
interface StockMyWatchlistDao {
    @Query("SELECT * FROM Stock_MyWatchlist ORDER BY stock_code ASC")
    fun observeAll(): Flow<List<StockMyWatchlistEntity>>

    @Query("SELECT * FROM Stock_MyWatchlist ORDER BY stock_code ASC")
    suspend fun getAll(): List<StockMyWatchlistEntity>

    @Query("SELECT stock_code FROM Stock_MyWatchlist")
    ...
    @Query("UPDATE Stock_MyWatchlist SET Judgment_Result = :result WHERE stock_code = :code")
    suspend fun updateJudgmentResult(code: String, result: String)

    @Query("UPDATE Stock_MyWatchlist SET current_price = :price WHERE stock_code = :code")
    suspend fun updatePrice(code: String, price: Double)

    @Query("DELETE FROM Stock_MyWatchlist WHERE stock_code NOT IN (:keepCodes)")
    suspend fun deleteExcept(keepCodes: List<String>)

    @Query("DELETE FROM Stock_MyWatchlist WHERE stock_code = :code")
